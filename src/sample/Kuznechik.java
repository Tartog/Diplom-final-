...
public class Kuznechik {
    ...
    Kuznechik(String key)
    {
        key_1 = key.substring(0, 16).getBytes();
        key_2 = key.substring(16, 32).getBytes();

        GOST_Kuz_Expand_Key(key_1, key_2);
    }

    public String encr(String mes)
    {
        s = mes;
        String res1 = "";
        ...
        int start = 0;
        int end = 16;
        int numBlocks = s.length() / 16;
        for(int i = 0;i < numBlocks;i++) {
            blk = s.substring(start, end).getBytes();
            start += 16;
            end += 16;
            byte[] encriptBlok = GOST_Kuz_Encript(blk);
            res1 = res1 + DatatypeConverter.printHexBinary(encriptBlok);
        }
        return res1;
    }

    public String decr(String mes)
    {
        String res1 = "";
        int start = 0;
        int end = 32;
        int numBlocks = mes.length() / 32;
        for(int i = 0;i < numBlocks;i++) {
            byte[] tmp = GOST_Kuz_Decript(DatatypeConverter.parseHexBinary(mes.substring(start, end)));
            start += 32;
            end += 32;
            res1 = res1 + new String (tmp, StandardCharsets.UTF_8);
        }
        return res1;
    }
}
